...
export class Migration20230516112117_Create_Payment extends Migration {
    async up(): Promise<void> {
        ...
        this.addSql(
            'alter table "payments" add constraint "payments_account_id_foreign" foreign key ("account_id") references "accounts" ("id") on update cascade;',
        );
        this.addSql(
            'alter table "payments" add constraint "payments_payment_type_id_foreign" foreign key ("payment_type_id") references "payment_types" ("id") on update cascade;',
        );
        this.addSql(
            'alter table "payments" add constraint "payments_season_id_foreign" foreign key ("season_id") references "seasons" ("id") on update cascade;',
        );
    }
